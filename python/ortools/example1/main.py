...
# for those less familiar with optimization frameworks - let us
#  solve an instance of the NP-hard Knapsack Problem. This classic 
#  optimization problem requires selecting a subset of items, each 
#  with a specific weight and value, to maximize the total value 
#  without exceeding a weight limit. Although a recursive algorithm 
#  is easy to implement, 100 items yield approximately 2100≈1030 
#  possible solutions. Even with a supercomputer performing 1018 
#  operations per second, it would take more than 31,000 years 
...
# print(f"accumulated_weight: {accumulated_weight}")
model.add(accumulated_weight <= capacity)
accumulated_value = sum(x * v for x, v in zip(xs, values))
# print(f"accumulated_value: {accumulated_value}")
