...
print ('the counter was increased! '+ str(r_server.get('counter'))) #notice that the key is increased now

r_server.decr('counter') #we decrease the key value by 1, has to be int
print ('the counter was decreased! '+ str(r_server.get('counter'))) #the key is back to normal
...
print ('our redis list len is: %s'% str(r_server.llen('list1'))) #with llen we get our redis list size right from redis

print ('at pos 1 of our list is: %s'% str(r_server.lindex('list1', 1))) #with lindex we query redis to tell us which element is at pos 1 of our list

'''sets perform identically to the built in Python set type. Simply,
sets are lists but, can only have unique values.'''

r_server.sadd("set1", "el1")
r_server.sadd("set1", "el2")
r_server.sadd("set1", "el2")

print ('the member of our set are: %s'% str(r_server.smembers("set1")))
