# Receta — Bootstrap inicial con ETL externa + Debezium sin gap (MySQL 8)
...
Esta variante está escrita contra el stack `mysql8` del lab (`cdc-lab`). La terminología clave en MySQL 8 utiliza los verbos modernos `REPLICA` y `SOURCE` (introducidos en 8.0.22+), reemplazando los antiguos `SLAVE` y `MASTER`.
...
```mermaid
flowchart LR
    Primary[(MySQL 8<br/>Primary / Source)] -->|binlog<br/>replication| Replica[(MySQL 8<br/>Replica<br/>congelado durante ETL)]
...
sequenceDiagram
    ...
    participant Dbz as Debezium 8
    participant Rep as mysql-replica-8
    participant Pri as mysql-primary-8
    participant Dest as DB Destino
    participant State as dbz_state_8
    ...
    Op->>Rep: START REPLICA
    Op->>State: UPDATE offset_storage.gtids
    Op->>Dbz: snapshot.mode=schema_only (o no_data)
...
- Debezium del stack 8 (`debezium-server-8`) puede estar prendido o apagado al empezar — el procedimiento lo apaga al inicio para evitar carreras.
- DB destino existe y está vacía (o el operador acepta `UPSERT` por PK).
- `gtid_mode=ON` y `enforce_gtid_consistency=ON` están habilitados en el primary y el replica (default en MySQL 8 si el lab lo configuró así).
...
En MySQL 8 el verbo es `REPLICA`:
...
kubectl exec -n ${NS} ${REPLICA_POD} -- \
  ...
  | grep -E "(Relay_Source_Log_File|Exec_Source_Log_Pos|Executed_Gtid_Set)" \
...
Mientras `STOP REPLICA` está vigente, el replica es una snapshot estática consistente. La tabla se puede leer como el operador prefiera:
...
**Importante:** no se debe hacer `START REPLICA` aún. La replicación debe permanecer pausada hasta que la ETL termine de leer.
...
## Paso 8 — Configurar Debezium en modo `schema_only` (o `no_data`)

Sin esto, al arrancar Debezium podría intentar hacer su propio snapshot y reemitir todas las filas que la ETL ya cargó.

En Debezium 2.5+ (típico con MySQL 8) el nombre canónico es `no_data`, aunque `schema_only` sigue funcionando como alias:

```bash
# Patch del ConfigMap debezium-config-8
kubectl get configmap debezium-config-${STACK} -n ${NS} -o yaml \
  | sed 's/debezium.source.snapshot.mode=initial/debezium.source.snapshot.mode=no_data/' \
  | kubectl apply -f -

# Verificar:
kubectl get configmap debezium-config-${STACK} -n ${NS} -o jsonpath='{.data.application\.properties}' \
  | grep snapshot.mode
# Esperado: debezium.source.snapshot.mode=no_data
```

> Si los logs muestran `Unknown snapshot mode 'no_data'`, se está corriendo Debezium < 2.5 y el operador debe usar `schema_only`.
...
El operador inspecciona los logs del sink **desde el arranque** de Debezium y busca eventos `op=r` (read del snapshot). En `snapshot.mode=no_data` no debería haber ningún `op=r`:
...
Si los pasos 6–8 resultan incómodos (manipular el offset a mano es frágil), Debezium 2.x+ ofrece un mecanismo de re-snapshot **on-demand** que se dispara via una "signal table" en el origen, sin tocar el offset. Esta alternativa es especialmente recomendable con MySQL 8 + Debezium 2.5+, donde la integración está madura.
...
kubectl get configmap debezium-config-${STACK} -n ${NS} -o yaml \
  | sed 's/debezium.source.snapshot.mode=no_data/debezium.source.snapshot.mode=initial/' \
  | kubectl apply -f -
...
|---|---|---|
| Debezium arranca y reemite todas las filas como `op=r` | `snapshot.mode` quedó en `initial` | Volver al Paso 8: cambiar a `no_data` (o `schema_only`) |
...
| Logs de Debezium: `Unknown snapshot mode 'no_data'` | Versión de Debezium < 2.5 que solo acepta `schema_only` | Cambiar `no_data` por `schema_only` en el ConfigMap |
| Logs de Debezium: `Cannot find binlog file mysql-bin.NNNNN` | El binlog se purgó (excede `binlog_expire_logs_seconds`) antes de que Debezium lo leyera | El GTID capturado quedó fuera de retención. Hay que rehacer la ETL desde cero. Considerar aumentar `binlog_expire_logs_seconds` durante la migración. |
...
| `STOP REPLICA` en **replica** (este procedimiento) | ❌ | ❌ | ✅ |
...
La única "desventaja" es que el replica se queda atrás durante la ETL, pero eso es transparente para los consumidores (no hay nada apuntando al replica salvo Debezium, y este está apagado). Cuando se ejecuta `START REPLICA` se pone al día en segundos para una tabla típica.

---

## Apéndice — Diferencias clave de MySQL 8 vs MySQL 5.7

| Concepto | MySQL 8 (este doc) | MySQL 5.7 |
|---|---|---|
| Pausar replicación | `STOP REPLICA` | `STOP SLAVE` |
| Reanudar replicación | `START REPLICA` | `START SLAVE` |
| Ver estado | `SHOW REPLICA STATUS\G` | `SHOW SLAVE STATUS\G` |
| Hilo I/O | `Replica_IO_Running` | `Slave_IO_Running` |
| Hilo SQL | `Replica_SQL_Running` | `Slave_SQL_Running` |
| Lag | `Seconds_Behind_Source` | `Seconds_Behind_Master` |
| Binlog file en replica status | `Relay_Source_Log_File` | `Relay_Master_Log_File` |
| Snapshot mode preferido (Debezium) | `no_data` (2.5+) | `schema_only` |

> Los verbos `SLAVE` siguen funcionando en MySQL 8 como alias deprecados; pueden desaparecer en versiones futuras. Se recomienda usar la terminología nueva (`REPLICA`, `SOURCE`) para código nuevo.
